    ...
    INotificationService notificationService) : PageViewModelBase("Dashboard")
{
    ...
    public async Task LoadAsync()
    {
        ...
        ExpiryAlerts = await dbContext.StockBatches.CountAsync(x => x.Quantity > 0 && x.ExpiryDate >= today && x.ExpiryDate <= expiryDate, default);

        Status = $"Loaded dashboard for last 30 days";
    }
